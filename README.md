...
**Option A — Pull from GHCR (fastest, no file download):**
```bash
...
Download from GitHub Releases: **[Download openfang-image.tar.gz](https://github.com/Clawbuilders/openfang-setup/releases/latest)**
...
### Step 2 — Clone OpenFang (for config files)
...
Create a `.env` file in the `openfang` directory:
...
# Windows (PowerShell)
"NVIDIA_API_KEY=nvapi-your-key-here" | Out-File -Encoding utf8 .env
```

Replace `nvapi-your-key-here` with your key from build.nvidia.com.

---

### Step 4 — Configure OpenFang for NVIDIA NIM

The repo includes a ready-made NVIDIA NIM config. Verify `openfang.toml` contains:
...
docker run -d \
  --name openfang \
  -p 4200:4200 \
  -v ./openfang.toml:/data/config.toml:ro \
  --env-file .env \
